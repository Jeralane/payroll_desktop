...
namespace PayrollSystem.Views.Detachments
{
    ...
    {
        private DetachmentsViewModel _viewModel = new DetachmentsViewModel();
        private DetachmentsViewModel _lookup = new DetachmentsViewModel();
        ...
        public void Refresh()
        {
            var tempModel = new DetachmentsViewModel();
            tempModel.Collection = new ObservableCollection<Detachment>();
            ...
            else
            {
                var filteredItem = from item in _lookup.Collection
                                   where (item.Name.ToLower() + item.Address.ToLower()).Contains(searchItem.ToLower())
                                   select item;
        ...
        public void Edit()
        {
            ...
            var currentItem = new Detachment();
            currentItem.Read(_viewModel.SelectedItem.Id);
            var view = new DetachmentView(currentItem);
            if (view.ShowDialog() == true)
            {
