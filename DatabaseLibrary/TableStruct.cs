...
namespace DatabaseLibrary
{
    internal struct TableStruct
    {
        ...
        public TableStruct(AbstractModel model)
        {
            ...
            {
                var customAttributes = propertyInfo.GetCustomAttributes(typeof(ColumnNameAttribute), false);

                var propertyName = propertyInfo.Name;
                var myPropInfo = type.GetProperty(propertyName);
                var propertyValue = myPropInfo.GetValue(model, null);
                var columnName = ((ColumnNameAttribute)(customAttributes[0])).ColumnName;

                Fields.Add(columnName, propertyValue);
            }

            // Table name will be the class name
            //Name = type.Name.ToTableName();
            Name =
                ((TableNameAttribute[]) Attribute.GetCustomAttributes(type, typeof (TableNameAttribute)))[0].TableName;
